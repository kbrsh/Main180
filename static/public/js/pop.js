  ...
  function draw() {
    ...
    for (var i = 0; i < particles.length; i++) {
      ...
      if(mouse.x > p.location.x - p.radius && mouse.x < p.location.x + p.radius && mouse.y > p.location.y - p.radius && mouse.y < p.location.y + p.radius) {
        ...
        p.speed.x += randomInt(-2.5, 2.5)
        p.speed.y += randomInt(-2.5, 2.5)
        score++;

      } 
      
      document.getElementById("growth").innerHTML = 'Growth - ' + score
      document.getElementById("popped").innerHTML = 'Popped - ' + popped
    }
  }
